...
const sites = fibonacci_sphere(150, false).map(spherical);
...
const projection = d3.geoOrthographic().rotate([init,0]);
...
function redrawPolygon(polygon) {
  ...
  .attr('fill', d => {
      const color = d.fill;
      const c = d.cartesian;
      const t = (2.2 -2.*c[1] -1.4*c[2])/6;
      return d3.interpolate('white', color)(t);
  })
  .attr('stroke', d => {
      const c = d.cartesian;
      const t = 0.2+(2.2 -2.*c[1] -1.4*c[2])/7;
